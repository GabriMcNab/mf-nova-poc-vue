...
const selectors = {
  title: "label[data-testid='form-title']",
  description: "p[data-testid='form-description']",
...
describe("FormSection", () => {
  test("it should mount and render correctly", () => {
    ...
    expect(wrapper.find(selectors.title).text()).toBe(
      "experience.test_id.title"
    );
    expect(wrapper.find(selectors.description).text()).toBe(
      "experience.test_id.description"
    );
  ...
  describe("when a maximum width for the slot is passed as prop", () => {
    test("it should apply it to the slot container", () => {
      ...
      expect(wrapper.find(selectors.slotContainer).attributes().style).toBe(
        "max-width: 100px;"
      );
    });
  });
