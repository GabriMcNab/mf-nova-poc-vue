<template>
  ...
  >
    ...
    >
      <div
        class="AdditionalServices__title"
      ...
      >
        ...
        <span
          ...
          class="AdditionalServices__title-filled"
          >{{ $t(`common.filled`) }}</span
        >
      </div>
      <NovaIcon
        name="chevron-down"
        class="AdditionalServices__right-icon"
        :open="open || null"
        :size="15"
      />
    </button>
    <div v-if="open" class="AdditionalServices__content" :aria-expanded="open">
      <p
        class="AdditionalServices__description"
...
const handleServiceSelect = (opt: Option) => {
  if (props.modelValue.includes(opt.value)) {
    const newServices = props.modelValue.filter(
