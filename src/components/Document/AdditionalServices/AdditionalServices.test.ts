...
describe("when a value is passed as selected prop", () => {
  test("the corresponding service should be selected", () => {
    const wrapper = mount(AdditionalServices, {
      props: {
        ...props,
        modelValue: [props.additionalServices[0].id],
        open: true,
      },
    });

    expect(wrapper.findAll(selectors.listItem)[0].attributes("selected")).toBe(
      "true"
    );
  });
  test("Next the title should appear the label common.filled", () => {
    const wrapper = mount(AdditionalServices, {
      props: { ...props, modelValue: [props.additionalServices[0].id] },
    });
    expect(wrapper.find(selectors.title).text()).toBe(
      "additional.services.test-id.title common.filled"
    );
  });
});
...
describe("when the user selects a service", () => {
  test("it should emit an event", async () => {
    ...
    expect(events).toBeTruthy();
    expect(events[0][0]).toStrictEqual([props.additionalServices[1].id]);
  });
});
...
describe("when the user click on Clear All", () => {
  test("it should emit an event", async () => {
    const wrapper = mount(AdditionalServices, {
      props: {
        ...props,
        modelValue: [props.additionalServices[1].id],
        open: true,
      },
    });

    expect(wrapper.findAll(selectors.listItem)[1].attributes("selected")).toBe(
      "true"
    );
