...
describe("NovaToast", () => {
  ...
  test("the visibility timeout order should be correct", async () => {
    ...
    expect(
      wrapper.emitted<NovaToastEvents>()["notification:visible"]
    ).toBeFalsy();
    ...
    expect(
      wrapper.emitted<NovaToastEvents>()["notification:visible"][0].toString()
    ).toBe("12345,true");

    expect(
      wrapper.emitted<NovaToastEvents>()["notification:visible"][1].toString()
    ).toBe("12345,false");

    expect(
      wrapper.emitted<NovaToastEvents>()["notification:delete"][0].toString()
    ).toBe("12345");
  });
  ...
  test("it should emit a notification close event with the notification id", async () => {
    ...
    expect(wrapper.emitted()["notification:delete"].toString()).toContain(
      "12345"
    );
  });
